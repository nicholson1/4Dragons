...
using static InputIcons.II_TextPrompt;

namespace InputIcons
{
    [CustomEditor(typeof(II_TextPrompt))]
    public class II_TextPromptEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            II_TextPrompt textPromptBehaviour = (II_TextPrompt)target;

            List<TextPromptData> savedPromptData = TextPromptData.CloneList(textPromptBehaviour.textPromptDatas);

            EditorGUI.BeginChangeCheck();

            if (textPromptBehaviour.textComponent == null)
            {
                EditorGUILayout.Space(10);
                EditorGUILayout.HelpBox("Assign a TMP component to display binding in scene", MessageType.Warning);
            }
            textPromptBehaviour.textComponent = (TMP_Text)EditorGUILayout.ObjectField("Text Component", textPromptBehaviour.textComponent, typeof(TMP_Text), true);

            EditorGUILayout.HelpBox("Type your text in the following field.\n\n" +
                "Instances of " + InputIconsManagerSO.TEXT_TAG_VALUE + " will be replaced with selected action references on this script (top to bottom).\n\n" +
                "Note: TMP_Assets need to first be generated for sprites to show up.", MessageType.Info);
            textPromptBehaviour.originalText = EditorGUILayout.TextArea(textPromptBehaviour.originalText);
            ...
            foreach (TextPromptData sData in savedPromptData.ToList())
            {
                ...
                if (sData.actionReference != null)
                {
                    ...
                    if (sData.showAllInfo)
                    {
                        ...
                        {
                            sData.bindingSearchStrategy = (TextPromptData.BindingSearchStrategy)EditorGUILayout.EnumPopup("Search Binding By", sData.bindingSearchStrategy);
                            if (sData.bindingSearchStrategy == TextPromptData.BindingSearchStrategy.BindingType)
                            {
                            ...
                            else if (sData.bindingSearchStrategy == TextPromptData.BindingSearchStrategy.BindingIndex)
                            {
