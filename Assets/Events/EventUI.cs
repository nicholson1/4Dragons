        ...
        for (int i = 0; i < outcomes.Length; i++)
        {
            if (outcomes[i] > EOutcome.None && chances[i] > 0f)
            {
                ...
                if (outcomes[i] != EOutcome.Buff && outcomes[i] != EOutcome.Debuff)
                {
                    ...
                    //change display name text to icon here?
    ...
    private void ActivateOutcome(OutcomeInfo outcomeInfo)
    {
        ...
        switch (outcome)
        {
            ...
            case EOutcome.RelicGet:
                SelectionManager._instance.CreateChestReward(false, SelectionManager.ChestType.Relic, SelectionManager.ChestType.None, true);
                UIController._instance.CloseMysteryAndOpenRelicSelection();
            ...
            case EOutcome.EquipmentGet:
                ...
                UIController._instance.CloseMysteryScreenAndOpenLoot();
                break;
        ...
        if(outcome != EOutcome.EquipmentGet && outcome != EOutcome.RelicGet)
        {
            UIController._instance.CloseMysteryScreenAndOpenMap();
            return;
        }       
